...
openssl pkcs12 -export -in tmp/myservice.crt -inkey tmp/myservice.key -certfile tmp/cxfca.crt -name "myservicekey" -out servicestore.p12 -passout pass:sspass -keypbe aes-256-cbc -certpbe aes-256-cbc
keytool -import -trustcacerts -alias mystskey -file tmp/mysts.crt -noprompt -keystore servicestore.p12  -storepass sspass
